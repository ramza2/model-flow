...
SOURCE_MYSQL_HOST_PORT="${SOURCE_MYSQL_HOST_PORT:-3307}"
SOURCE_MARIADB_HOST_PORT="${SOURCE_MARIADB_HOST_PORT:-3308}"
SOURCE_MSSQL_HOST_PORT="${SOURCE_MSSQL_HOST_PORT:-14333}"
SOURCE_ORACLE_HOST_PORT="${SOURCE_ORACLE_HOST_PORT:-15211}"
...
print("SOURCE_MYSQL_ROOT_PASSWORD=" + urlsafe(36))
print("SOURCE_MYSQL_USER=" + identifier("mysql"))
print("SOURCE_MYSQL_PASSWORD=" + urlsafe(36))
print("SOURCE_MYSQL_DB=" + identifier("mysql"))
print("SOURCE_MARIADB_ROOT_PASSWORD=" + urlsafe(36))
print("SOURCE_MARIADB_USER=" + identifier("maria"))
print("SOURCE_MARIADB_PASSWORD=" + urlsafe(36))
print("SOURCE_MARIADB_DB=" + identifier("maria"))
# SQL Server SA/login passwords need complexity (upper/lower/digit/symbol).
print("SOURCE_MSSQL_SA_PASSWORD=Ms1." + urlsafe(28))
print("SOURCE_MSSQL_USER=" + identifier("mssql"))
print("SOURCE_MSSQL_PASSWORD=Ms1." + urlsafe(28))
print("SOURCE_MSSQL_DB=" + identifier("mssql"))
# Oracle Free (gvenzl) passwords need complexity; service name is fixture-fixed.
print("SOURCE_ORACLE_SYSTEM_PASSWORD=Or1." + urlsafe(28))
print("SOURCE_ORACLE_APP_USER=" + identifier("oraapp"))
print("SOURCE_ORACLE_APP_PASSWORD=Or1." + urlsafe(28))
print("SOURCE_ORACLE_USER=" + identifier("oraro"))
print("SOURCE_ORACLE_PASSWORD=Or1." + urlsafe(28))
print("SOURCE_ORACLE_SERVICE_NAME=FREEPDB1")
PY
)"
...
if [[ "$NON_INTERACTIVE" != true ]]; then
  ...
  prompt_value SOURCE_MYSQL_USER "Optional source MySQL user"
  prompt_secret SOURCE_MYSQL_PASSWORD "Optional source MySQL password"
  prompt_secret SOURCE_MYSQL_ROOT_PASSWORD "Optional source MySQL root password"
  prompt_value SOURCE_MYSQL_DB "Optional source MySQL database"
  prompt_value SOURCE_MARIADB_USER "Optional source MariaDB user"
  prompt_secret SOURCE_MARIADB_PASSWORD "Optional source MariaDB password"
  prompt_secret SOURCE_MARIADB_ROOT_PASSWORD "Optional source MariaDB root password"
  prompt_value SOURCE_MARIADB_DB "Optional source MariaDB database"
  prompt_value SOURCE_MSSQL_USER "Optional source SQL Server user"
  prompt_secret SOURCE_MSSQL_PASSWORD "Optional source SQL Server password"
  prompt_secret SOURCE_MSSQL_SA_PASSWORD "Optional source SQL Server SA password"
  prompt_value SOURCE_MSSQL_DB "Optional source SQL Server database"
  prompt_value SOURCE_ORACLE_USER "Optional source Oracle reader user"
  prompt_secret SOURCE_ORACLE_PASSWORD "Optional source Oracle reader password"
  prompt_secret SOURCE_ORACLE_SYSTEM_PASSWORD "Optional source Oracle SYSTEM password"
  prompt_value SOURCE_ORACLE_APP_USER "Optional source Oracle app user"
  prompt_secret SOURCE_ORACLE_APP_PASSWORD "Optional source Oracle app password"
  prompt_value SOURCE_ORACLE_SERVICE_NAME "Optional source Oracle service name"
  ...
  prompt_value SOURCE_MYSQL_HOST_PORT "Source MySQL host port"
  prompt_value SOURCE_MARIADB_HOST_PORT "Source MariaDB host port"
  prompt_value SOURCE_MSSQL_HOST_PORT "Source SQL Server host port"
  prompt_value SOURCE_ORACLE_HOST_PORT "Source Oracle host port"
...
for variable in \
  ...
  SOURCE_MYSQL_ROOT_PASSWORD SOURCE_MYSQL_USER SOURCE_MYSQL_PASSWORD SOURCE_MYSQL_DB \
  SOURCE_MARIADB_ROOT_PASSWORD SOURCE_MARIADB_USER SOURCE_MARIADB_PASSWORD SOURCE_MARIADB_DB \
  SOURCE_MSSQL_SA_PASSWORD SOURCE_MSSQL_USER SOURCE_MSSQL_PASSWORD SOURCE_MSSQL_DB \
  SOURCE_ORACLE_SYSTEM_PASSWORD SOURCE_ORACLE_APP_USER SOURCE_ORACLE_APP_PASSWORD \
  SOURCE_ORACLE_USER SOURCE_ORACLE_PASSWORD SOURCE_ORACLE_SERVICE_NAME \
  CORS_ORIGINS \
  POSTGRES_HOST_PORT SOURCE_POSTGRES_HOST_PORT \
  SOURCE_MYSQL_HOST_PORT SOURCE_MARIADB_HOST_PORT SOURCE_MSSQL_HOST_PORT \
  SOURCE_ORACLE_HOST_PORT \
...
for variable in \
  POSTGRES_USER POSTGRES_DB SOURCE_POSTGRES_USER SOURCE_POSTGRES_DB \
  SOURCE_MYSQL_USER SOURCE_MYSQL_DB SOURCE_MARIADB_USER SOURCE_MARIADB_DB \
  SOURCE_MSSQL_USER SOURCE_MSSQL_DB \
  SOURCE_ORACLE_APP_USER SOURCE_ORACLE_USER SOURCE_ORACLE_SERVICE_NAME; do
  if [[ ! "${!variable}" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then
    echo "$variable must be a valid unquoted SQL identifier." >&2
    exit 1
  fi
done

for variable in \
  POSTGRES_PASSWORD MINIO_ROOT_PASSWORD SOURCE_POSTGRES_PASSWORD \
  SOURCE_MYSQL_ROOT_PASSWORD SOURCE_MYSQL_PASSWORD \
  SOURCE_MARIADB_ROOT_PASSWORD SOURCE_MARIADB_PASSWORD \
  SOURCE_MSSQL_SA_PASSWORD SOURCE_MSSQL_PASSWORD \
  SOURCE_ORACLE_SYSTEM_PASSWORD SOURCE_ORACLE_APP_PASSWORD SOURCE_ORACLE_PASSWORD; do
  if [[ ! "${!variable}" =~ ^[A-Za-z0-9_.~-]+$ ]]; then
...
for variable in \
  ...
  MINIO_ROOT_PASSWORD SOURCE_POSTGRES_PASSWORD \
  SOURCE_MYSQL_ROOT_PASSWORD SOURCE_MYSQL_PASSWORD \
  SOURCE_MARIADB_ROOT_PASSWORD SOURCE_MARIADB_PASSWORD \
  SOURCE_MSSQL_SA_PASSWORD SOURCE_MSSQL_PASSWORD \
  SOURCE_ORACLE_SYSTEM_PASSWORD SOURCE_ORACLE_APP_PASSWORD SOURCE_ORACLE_PASSWORD; do
...
SOURCE_MARIADB_ROOT_PASSWORD=$SOURCE_MARIADB_ROOT_PASSWORD
SOURCE_MARIADB_USER=$SOURCE_MARIADB_USER
SOURCE_MARIADB_PASSWORD=$SOURCE_MARIADB_PASSWORD
SOURCE_MARIADB_DB=$SOURCE_MARIADB_DB
SOURCE_MSSQL_SA_PASSWORD=$SOURCE_MSSQL_SA_PASSWORD
SOURCE_MSSQL_USER=$SOURCE_MSSQL_USER
SOURCE_MSSQL_PASSWORD=$SOURCE_MSSQL_PASSWORD
SOURCE_MSSQL_DB=$SOURCE_MSSQL_DB
SOURCE_ORACLE_SYSTEM_PASSWORD=$SOURCE_ORACLE_SYSTEM_PASSWORD
SOURCE_ORACLE_APP_USER=$SOURCE_ORACLE_APP_USER
SOURCE_ORACLE_APP_PASSWORD=$SOURCE_ORACLE_APP_PASSWORD
SOURCE_ORACLE_USER=$SOURCE_ORACLE_USER
SOURCE_ORACLE_PASSWORD=$SOURCE_ORACLE_PASSWORD
SOURCE_ORACLE_SERVICE_NAME=$SOURCE_ORACLE_SERVICE_NAME
POSTGRES_HOST_PORT=$POSTGRES_HOST_PORT
SOURCE_POSTGRES_HOST_PORT=$SOURCE_POSTGRES_HOST_PORT
SOURCE_MYSQL_HOST_PORT=$SOURCE_MYSQL_HOST_PORT
SOURCE_MARIADB_HOST_PORT=$SOURCE_MARIADB_HOST_PORT
SOURCE_MSSQL_HOST_PORT=$SOURCE_MSSQL_HOST_PORT
SOURCE_ORACLE_HOST_PORT=$SOURCE_ORACLE_HOST_PORT
