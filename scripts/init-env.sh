...
for variable in \
  ...
  SOURCE_MYSQL_HOST_PORT SOURCE_MARIADB_HOST_PORT \
...
for variable in \
  ...
  SOURCE_MYSQL_USER SOURCE_MYSQL_DB SOURCE_MARIADB_USER SOURCE_MARIADB_DB; do
...
for variable in \
  ...
  SOURCE_MARIADB_ROOT_PASSWORD SOURCE_MARIADB_PASSWORD; do
  if [[ ! "${!variable}" =~ ^[A-Za-z0-9_.~-]+$ ]]; then
