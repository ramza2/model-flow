...
## D-041: Microsoft SQL Server uses mssql+pyodbc + ODBC Driver 18 with application-level read-only validation

- **Context:** Phase 4-C adds SQL Server after MySQL/MariaDB. SQL Server does not provide a PostgreSQL/MySQL-equivalent transaction READ ONLY statement; `ApplicationIntent=ReadOnly` is a routing intent, not a write-blocking security boundary. ODBC Driver 18 also changes TLS defaults and must not be left implicit. SQL Server rejects `LIMIT`, so the shared LIMIT preview wrapper cannot be used.
- **Choice:** Add `DataSourceType.mssql` with UI label “Microsoft SQL Server”, default port 1433, driver `mssql+pyodbc`, and Microsoft ODBC Driver 18 in the backend image. Host/Port and Connection URL modes reuse existing SQL secret/`connection_mode` semantics; allowed URL schemes are only `mssql://` and `mssql+pyodbc://` (bare `mssql://` normalizes to `mssql+pyodbc://`). Typed config defaults `encrypt=true` and `trust_server_certificate=false`; disposable Compose fixtures may set trust for self-signed certificates. Preview bounds rows with `fetchmany` instead of rewriting SQL with `LIMIT`. `MssqlConnector` applies stricter validation (mutations, `EXEC`/`EXECUTE`, `SELECT INTO`, `WITH … UPDATE/DELETE/INSERT/MERGE`, multi-statements). Disposable fixture `mcr.microsoft.com/mssql/server:2022-CU27-ubuntu-22.04` seeds `dbo.customers` and a SELECT-only login as an additional lab defense, not as a claim that every production account is read-only.
- **Consequences:** PostgreSQL/MySQL transaction READ ONLY behavior is unchanged. Certificate upload / custom CA / client-cert management, Windows Authentication, CDC, and write-back remain out of scope. Oracle remains Phase 4-D.

## D-042: Oracle uses oracle+oracledb Thin mode with service_name and SET TRANSACTION READ ONLY

- **Context:** Phase 4-D adds Oracle after SQL Server. Instant Client / Thick mode would enlarge the backend image and complicate CI. Oracle supports real `SET TRANSACTION READ ONLY` (unlike SQL Server). Host/Port UX must not confuse SID with service name.
- **Choice:** Add `DataSourceType.oracle` with UI label “Oracle Database”, default port 1521, driver `oracle+oracledb`, and pinned `oracledb` Thin mode (no Instant Client). Host/Port stores `service_name` (not database path). Connection URL schemes are only `oracle://` and `oracle+oracledb://` (bare `oracle://` normalizes); allowlisted query key is `service_name` only; username is required. Import/preview transactions begin then issue `SET TRANSACTION READ ONLY` as the first statement. Preview bounds rows with `fetchmany`. Disposable fixture `gvenzl/oracle-free:23.26.3-slim-faststart` seeds `CUSTOMERS` and a SELECT-only reader. SID typed mode, wallets, thick mode, and TNS_ADMIN configs stay out of scope.
- **Consequences:** PostgreSQL/MySQL/MSSQL connectors are unchanged. Oracle Instant Client packaging and Autonomous Wallet flows need an explicit later phase if required.

## D-043: Phase 4-E Oracle UDF allowlist and duplicate query fail-closed

- **Context:** Phase 4-D Final Review left non-blocking debt: Oracle `SET TRANSACTION READ ONLY` does not stop `PRAGMA AUTONOMOUS_TRANSACTION` functions invoked from SELECT; duplicate `service_name` query params collapsed last-wins; URL mode could retain stale Host/Port `service_name`; PostgreSQL DSN accepted cross-dialect schemes.
