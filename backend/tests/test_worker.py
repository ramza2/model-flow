...
def test_process_import_job_materializes_oracle_connector_result(monkeypatch):
    with TestingSessionLocal() as db:
        project = Project(name="oracle-import-worker")
        db.add(project)
        db.flush()
        dataset = Dataset(project_id=project.id, name="customers")
        db.add(dataset)
        ...
        source = DataSource(
            ...
            config_json=(
                '{"host":"oracle-source","port":1521,"service_name":"FREEPDB1",'
                '"user":"reader"}'
            ),
            is_active=True,
        )
        ...
        job = DataImportJob(
            ...
            data_source_id=source.id,
            dataset_id=dataset.id,
            query_or_table="MODELFLOW.CUSTOMERS",
            status=JobStatus.running,
        )
        db.add(job)
        db.commit()
    ...
    class FakeConnector:
        ...
        def read_frame(self, resource):
            assert resource == "MODELFLOW.CUSTOMERS"
            return pd.DataFrame(
                [{"id": 1, "name": "Ada", "segment": "enterprise"}]
            )
    ...
    def fake_materialize(
        ...
        *,
        ...
        version = DatasetVersion(
            ...
            project_id=dataset.project_id,
            version=1,
            object_key="datasets/oracle-import.csv",
            original_filename=filename,
            format=file_format,
            source_type=source_type,
            data_source_id=data_source_id,
    ...
    with TestingSessionLocal() as db:
        ...
        assert job.status == JobStatus.succeeded
        version = db.get(DatasetVersion, job.dataset_version_id)
        assert version.source_type == "oracle"
        assert version.data_source_id == source_id
        assert version.import_job_id == job_id

    assert captured["source_type"] == "oracle"
    assert captured["file_format"] == "csv"
    assert b"Ada" in captured["payload"]
